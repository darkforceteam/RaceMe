...
class TrackingViewController: UIViewController {
    ...
    @IBAction func startRun(_ sender: UIButton) {
        ...
        newWorkOut = WorkoutObject.init(userId: "123456", type: "RUN", startTime: Date(), routeId: "1", startLocation: nil)
    ...
    @IBAction func stopRun(_ sender: UIButton) {
        if workOutStatus == WO_STARTED
        {
            ...
            timer.invalidate()
            newWorkOut?.completed(endTime: Date(), endLocation: nil, distanceKm: self.distanceKm, duration: duration)
    ...
    func saveWorkout(routeID: String){
        ...
        let woRef = ref.child("\(Constants.Workout.TABLE_NAME)")
        woRef.child("\(routeID)").setValue(newWorkOut?.toAnyObject())
